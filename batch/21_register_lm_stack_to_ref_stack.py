# 21_register_lm_stack_to_ref_stack_batch.py
import os
import sys
...
STEP_NAME = '21_register_lm_stack_to_ref_stack'
...
def process_sample(sample_id, db_path, update_all=False):
    try:
        ...
        anatomy_ref_aligned_path = os.path.join(exp.paths.anatomy_path, "aligned_to_reference")
        if len(os.listdir(anatomy_ref_aligned_path))>0:
...
def main():
    print("WARNING!! This script only checks if there are files in the anatomy/aligned_to_reference folder.")
